...
class User(Base):
    ...
    telegram_user_id = mapped_column(BigInteger, primary_key=True)
...
class Manager(Base):
    ...
    is_manager: Mapped[bool] = mapped_column(Boolean, default=True)
...
class Ticket(Base):
    ...
    status: Mapped[str] = mapped_column(String, default=TicketStatus.WAITING_MANAGER)
    assigned_manager_telegram_id: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
...
class ManagerState(Base):
    ...
    manager_telegram_id = Column(BigInteger, primary_key=True)
